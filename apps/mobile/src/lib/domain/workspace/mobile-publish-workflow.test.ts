...
import type { PublishIntent } from "./mobile-publish-workflow-model";

// Ported verbatim (import paths only adjusted) from web's
// `publish-workflow.test.ts`
// (`product-client/src/lib/domain/workspaces/creation/publish-workflow.test.ts`)
// as the parity guide for `buildPublishViewState`. Mobile's call site
// (`mobile-publish-view.ts`) only ever passes `intent: "pull_request"`, but
// this suite exercises the full ported function across all three intents to
// prove the port is a faithful line-for-line mirror of web's.
...
describe("buildPublishViewState", () => {
  ...
  it("keeps a blank summary submittable for generation on an existing PR update", () => {
    ...
    // Blank summaries stay valid at THIS layer: `buildPublishViewState` is
    // the verbatim web port, which assumes leave-blank-to-generate (AI
    // magic). Mobile's own guard against a blank summary lives one layer up
    // in `mobile-publish-view.ts`'s `buildMobilePublishView` (see its module
    // doc — mobile doesn't wire AI generation, so it requires the field).
    expect(result.disabledReason).toBeNull();
    expect(result.primaryLabel).toBe("Commit and push");
