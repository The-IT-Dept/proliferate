...
/**
 * Row 29 — ported VERBATIM from web's `publish-workflow-labels.ts`
 * (`product-client/src/lib/domain/workspaces/creation/publish-workflow-labels.ts`):
 * same three functions, same copy, same branching, all three intents kept
 * (mobile's own call site always passes `intent: "pull_request"` — see
 * `mobile-publish-view.ts` — but the file itself stays a genuine port so it
 * can be diffed against web's directly).
 */
