...
/**
 * Row 29 — ported VERBATIM from web's `publish-workflow-steps.ts`
 * (`product-client/src/lib/domain/workspaces/creation/publish-workflow-steps.ts`):
 * same validation precedence and same ordered step derivation. Consumed by
 * `mobile-publish-workflow.ts`'s `buildPublishViewState` port, exactly as on
 * web.
 */
